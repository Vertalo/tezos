...
if ! sha256 -c "${sapling_output_parameters_sha256}"  "${OUTPUT_PARAMETERS_FILE}"  > /dev/null 2>&1 ; then
...
if ! sha256 -c "${sapling_spend_parameters_sha256}"  "${SPEND_PARAMETERS_FILE}"  > /dev/null 2>&1 ; then
